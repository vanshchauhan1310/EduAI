...
def generate_translated_document_docx(
    file_name: str,
    translated_text: str,
    source_language: str,
    target_language: str,
    ...
    """Generate an editable DOCX for translated document text."""
    doc = Document()
    ...
    meta_lines = [
        ...
        f"Language: {source_language} to {target_language}",
    ]
    if document_type:
        meta_lines.append(f"Document type: {document_type}")
    meta_lines.append(f"Generated: {date.today().strftime('%d %B %Y')}")

    for line in meta_lines:
        p = doc.add_paragraph(line)
        if p.runs:
            _set_font_for_run(p.runs[0], size=9)
    ...
    if footer.runs:
        _set_font_for_run(footer.runs[0], size=8)
