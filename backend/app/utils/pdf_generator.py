...
def generate_translated_document_pdf(
    file_name: str,
    translated_text: str,
    source_language: str,
    target_language: str,
    ...
    """Render translated document text as a formatted A4 PDF."""
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer, pagesize=A4,
        rightMargin=2.5 * cm, leftMargin=2.5 * cm,
    ...
    elements.extend(_letterhead(styles))
    elements.append(Paragraph("TRANSLATED DOCUMENT", styles["header"]))
    elements.append(Paragraph(f"Source file: {escape(file_name)}", styles["small"]))
    elements.append(Paragraph(f"Language: {source_language} to {target_language}", styles["small"]))
    if document_type:
        elements.append(Paragraph(f"Document type: {escape(document_type)}", styles["small"]))
    elements.append(Paragraph(f"Generated: {date.today().strftime('%d %B %Y')}", styles["small"]))
    elements.append(Spacer(1, 6 * mm))
    ...
    for line in translated_text.split("\n"):
        ...
        if not stripped:
            elements.append(Spacer(1, 3 * mm))
            continue
        elements.append(Paragraph(escape(stripped), styles["normal"]))
